...
function centerMiddleText(text, width, height) {
  return (
    ...
    'px; height: ' +
    height +
    'px;  #position: fixed; overflow: hidden;"><div style="#position: absolute; #top: 50%; display: table-cell; text-align:center; vertical-align: middle;"><div style=" #position: absolute; #top: -50%">' +
    text +
    '</div></div></div>'
  );
}
...
function center(text, width) {
  return (
    '<div style="width:' + width + 'px;"><center>' + text + '</center></div>'
  );
}
...
function rectangle(width, height) {
  return (
    '<img src="images/pixel.gif" height="' +
    height +
    '" width="' +
    width +
    '" />'
...
function collision(inst1, inst2, newX, newY, shrinkw, shrinkh) {
  ...
  if (
    inst1.x + inst1.width + newx - sw >= inst2.x &&
    inst1.x + newx + sw <= inst2.x + inst2.width &&
    inst1.y + inst1.height + newy - sh >= inst2.y &&
    inst1.y + newy + sh <= inst2.y + inst2.height
  )
    return true;
  return false;
}
...
function collisionRect(inst1, x, y, width, height) {
  ...
  if (
    inst1.x + inst1.width >= x &&
    inst1.x <= x + w &&
    inst1.y + inst1.height >= y &&
    inst1.y <= y + h
  )
    return true;
  return false;
...
function distanceTo(inst1, inst2) {
  return distance(
    inst1.x + inst1.width / 2,
    inst1.y + inst1.height / 2,
    inst2.x + inst2.width / 2,
    inst2.y + inst2.height / 2
  );
}
...
function directionTo(inst1, inst2) {
  var x1 = inst1.x + inst1.width * 0.5,
    y1 = inst1.y + inst1.height * 0.5,
    x2 = inst2.x + inst2.width * 0.5,
    y2 = inst2.y + inst2.height * 0.5;
...
function blocksAt(x1, y1, x2, y2, x3, y3) {
  ...
  if (groups.length > 0) {
    for (var i = 0, il = blocks.length; i < il; i++) {
      ...
      if (
        (block.x == x1 && block.y == y1) ||
        (block.x == x2 && block.y == y2) ||
        (block.x == x3 && block.y == y3)
      )
        blocksA.push(block);
    }
  }
...
function play(sound, loop) {
  if (audioOn) {
    ...
    if (!loop) sound.play();
    else sound.play({ numberOfLoops: 100 });
  }
}
