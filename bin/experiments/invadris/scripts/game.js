...
function initGame() {
  ...
  scoreText.move = function() {
    if (
      ...
      var rem =
        'Cleared<br/><b>' +
        totalblockscleared +
        '</b><br/><br/>Remaining<br/><b>' +
        Math.max(10 - blockscleared, 0) +
        '</b>';
  ...
  pauseText = new Inst(
    ...
    0,
    '<div class = "paused"><b>' +
      centerMiddleText('PAUSED', GAMEWIDTH, GAMEHEIGHT) +
      '</b></div>',
    'topLayer'
  );
  pauseText.visible = false;
  pauseText.move = function() {
    if (this.visible) this.setAlpha(1);
  ...
  line.a = 0;
  line.fade = 0;
  line.fadespeed = 0.01 * FACTOR;
  line.out = true;
  line.alpha = line.fade;
  line.move = function() {
    this.alpha = (Math.sin(this.a++ / 10) * 0.4 + 0.8) * this.fade;
    if (this.out && mode != 2) {
      if (this.fade < 1) this.fade += this.fadespeed;
    } else {
      if (this.fade > 0) this.fade -= this.fadespeed;
      else this.fade = 0;
    }
    line1.move();
    line2.move();
  ...
  line1.move = function() {
    if (line.out && mode != 2) {
      if (this.x < SCREENLEFT) this.x += 0.25;
    } else if (this.x > SCREENLEFT - this.width && line.fade < 0.2)
      this.x -= 0.25;
    this.y = line.y;
  };

  line2 = new Instance(
    SCREENLEFT + GAMEWIDTH - 6 + 7,
    GAMEHEIGHT - GRIDSIZE * 3 - 6
  );
  line2.image(laser1, 6, 11);
  line2.move = function() {
    this.x = (line1.x - SCREENLEFT * 2 - GAMEWIDTH + this.width) * -1;
    this.y = line.y;
  };
  ...
  clearedText.move = function() {
    if (this.old != this.cleared) {
      this.text(
        '<div class = "paused"><font size="5">' +
          centerMiddleText(this.cleared, GAMEWIDTH, GAMEHEIGHT) +
          '</font></div>'
      );
      this.old = this.cleared;
    }
    if (this.fadein) {
  ...
  if (mode != 2) highscore = table.getScore(0);
  //!!
...
function pause() {
  if (!inMenu && fadealpha <= 0 && !endGame) {
    if (!paused) {
      ...
      can.drawImage(
        pausetext,
        SCREENWIDTH - pausetext.width,
        Math.floor(pausetext.height / 2)
      );
      updatePause();
    } else {
      clearInterval(interval);
...
function updatePause() {
  ...
  if (
    (touchX >= SCREENWIDTH - 32 && touchY <= 16) ||
    (mX >= SCREENWIDTH - 32 && mY <= 10)
  )
...
function update() {
    ...
    for (var i = 0, il = blocks.length; i < il; i++) {
      ...
      if (other.y >= 0) {
        for (var k = 0, kl = bullets.length; k < kl; k++)
          if (
            collision(other, bullets[k], 0, 0, 2, -12) &&
            (bullets[k].alive || typeof bullets[k].alive == 'undefined')
          ) {
            new Explosion(other.x, other.y, 0.5);
            other.destroy();
            if (typeof bullets[k].indestructable == 'undefined')
              bullets[k].destroy();
          }
        ...
        for (var k = 0, kl = enemybullets.length; k < kl; k++)
          if (collision(other, enemybullets[k], 0, 0, 2, -12)) {
            new Explosion(other.x, other.y, 0.5);
            other.destroy();
            if (typeof enemybullets[k].indestructable == 'undefined')
  ...
  if (!ship.flash && enemies.length > 0)
    for (var i = 0, il = enemies.length; i < il; i++) {
      ...
      if (collision(ship, enemy, 0, 0, 4, 6)) {
        enemy.explode();
        if (ship.shield > 0) addScore(enemy.value * level);
        ship.explode();
      }
    }
  //check for squares
  if (!wave && (time == 0 || secondtime == 0)) {
    if (squares.length > 0) {
      ...
      if (squaredest > 0) {
        ...
        if (squaredest > 4) {
          clearedText.cleared = squaredest;
          clearedText.fadein = true;
        }
    ...
    if (squares.length > 0) {
      for (var i = 0, il = squares.length; i < il; i++) {
        if (squares[i][0].y < line.y && squares[i][0].y > lineA.y - GRIDSIZE) {
          var can = false;
          for (var j = 0, jl = squares[i].length; j < jl; j++)
            if (squares[i][j].oldy != squares[i][j].y || squares[i][j].y < 0) {
              can = true;
              break;
            } ///
          if (!can) {
    ...
    if (explode) {
      for (var k = 0; k < 15; k++)
        for (var i = 0, il = blocks.length; i < il; i++) blocks[i].move();
      dowork = true;
      explode = false;
      secondtime = 5;
    }
  ...
  if (wave || level >= 3) {
    if (randomTimer >= randomTime) {
      ...
      if (
        wave &&
        ((enemy1Num % 2 == 0 && enemy1Num > 0) ||
          (enemy2Num % 2 == 0 && enemy2Num > 0) ||
          (enemy3Num % 2 == 0 && enemy3Num > 0))
      )
        times = 2;
      ...
      for (var i = 0; i < times; i++) {
        var e = null;
        if (enemy1Num > 0 && Enemy.enemies < 4) {
          e = new Enemy(SCREENLEFT, -100);
          enemy1Num--;
        } else if (enemy2Num > 0 && Enemy2.enemies < 2) {
          e = new Enemy2(SCREENLEFT, -100);
          enemy2Num--;
        } else if (enemy3Num > 0 && Enemy3.enemies < 1) {
          e = new Enemy3(SCREENLEFT, -100);
          enemy3Num--;
        }
        ...
        if (e != null) {
          if (times == 1) e.x += (GAMEWIDTH - e.width) / 2;
            ...
            if (i == 0)
              e.x += Math.max(
                Math.floor(
                  (e.width + rand * (GAMEWIDTH + 1 - e.width) - e.width * 2) / 2
                ),
                0
              );
            else
              e.x += Math.min(
                Math.floor(
                  GAMEWIDTH - (e.width + rand * (GAMEWIDTH + 1 - e.width)) / 2
                ),
                GAMEWIDTH - e.width
              );
          }
      ...
      randomTime =
        Math.floor(
          Math.random() *
            Math.max((100 - level * 5) * (!wave + 1), FRAMERATE * (!wave + 1))
        ) +
        !wave * 100;
    } else randomTimer++;
    ...
    if (
      ...
      if (level == STARTLEVEL && !skipToLevel) {
      ...
      line.y =
        GAMEHEIGHT -
        GRIDSIZE * Math.min(2 + Math.floor((level + 1) / 2), 10) -
        6;
  ...
  if (!wave) {
    if (limiter < limitTime) limiter++;
      ...
      for (var i = 0; i < rInt; i++) g.rotate();

      var opens = openings(g.width);
      if (opens.length > 0) g.x = opens[randomInt(0, opens.length - 1)];
      else g.destroy();
      ...
      limiter = 0;
      limitTime = randomInt(
        Math.max(minTime + 100 - (level + 2) * 7, minTime),
        Math.max(50, 230 - (level + 2) * 10)
      );
    }
  }
  ...
  if (endGame) {
    if (endGameTimer < endGameTime) {
      endGameTimer++;
      clearInterval(interval);
      interval = setInterval(update, MSECS + endGameTimer * endGameTimer);
    } else {
      fadeOut(gameOver, 0.1);
      clearAccel();
    }
  }
...
function highscoreTable(index) {
  ...
  var color,
    hs,
    str = '',
    tables = [table, waveTable],
    tab = 0 + (mode == 2);
  if (tab > 0) str = 'Wave';
  var inst = new Inst(
    0,
    0,
    center('<div class = "hstitle"><b>' + str + 'Highscores</b></div>', 320)
  );
  for (var i = 0; i < 10; i++) {
      ...
      var inst = new Inst(
        ...
        '<div class = "hstext">' +
          center(
            img +
              '&nbsp;<b><font size = "+2">' +
              tables[tab].getName(i).fontcolor(color || cyan) +
              '</b>&nbsp;&nbsp;&nbsp;' +
              tables[tab].getScore(i).fontcolor(color || 'white') +
              '</font>&nbsp;' +
              img,
            320
          ) +
  ...
  pressText = new Inst(
    0,
    460,
    center('<font size = "-1"><b>Press To Continue</b></font>', 320)
  );
...
function updateHighscoreTable() {
  ...
  if (pressingScreen()) fadeOut(menu);
}
...
function updateFadeIn(funct) {
  ...
  if (fadewait > 0) {
    fadewait--;
    setTimeout('updateFadeIn(' + funct + ')', MSECS);
  } else {
    if (fadealpha > 0) {
      fadealpha -= fadespeed;
      setTimeout('updateFadeIn(' + funct + ')', MSECS);
    } else {
      interval = setInterval(funct, MSECS);
    }
  }
}
...
function updateFadeOut(funct) {
  ...
  if (fadewait > 0) {
    fadewait--;
    setTimeout('updateFadeOut(' + funct + ')', MSECS);
  } else {
    if (fadealpha < 1) {
      fadealpha += fadespeed;
      setTimeout('updateFadeOut(' + funct + ')', MSECS);
...
function menu() {
  ...
  new Button(xx, yy - 30, [button1a, button1b], function() {
    fadeOut(initGame);
  });
  var b = new Button(
    xx + 24,
    yy + 10,
    [button2aa, button2cb, button2ba, button2ab, button2ca, button2bb],
    function() {
      if ((skipToLevel && this.index == 0) || waveMode) this.cycle();
      else if (this.index == 2 && !waveMode) {
        this.index = 4;
        this.cycle();
      }
      setMode(++mode);
  ...
  var oldmode = mode;
  for (var i = 0; i < mode; i++) b.cycle();
  setMode(oldmode);
  new Button(xx, yy + spacing, [button7a, button7b], function() {
    score = -1;
    fadeOut(gameOver);
  });
  new Button(xx, yy + spacing * 2, [button8a, button8b], function() {
    fadeOut(tutorial);
  });
  ...
  if (tellthem) {
    try {
      navigator.notification.alert(
        'New Mode Unlocked!',
        'Click the button under "Play" to change modes',
        'Dang!'
      );
    } catch (e) {
...
function option() {
  ...
  b = new Button(
    xx,
    yy + spacing * 0,
    [button4a1, button4b1, button4a2, button4b2],
    function() {
      setKeypadOn(!keypadOn);
      this.cycle();
    }
  );
  if (!keypadOn) b.cycle();
  b = new Button(
    xx,
    yy + spacing * 1,
    [button6a1, button6b1, button6a2, button6b2],
    function() {
      setAudioOn(!audioOn);
      this.cycle();
    }
  ...
  b = new Button(
    xx,
    yy + spacing * 2,
    [button3a1, button3b2, button3a2, button3b1],
    function() {
  ...
  new Button(xx, yy + spacing * 3, [button5a, button5b], function() {
    fadeOut(menu);
  });
...
function tutorial() {
  ...
  topCan.drawImage(
    loading,
    (SCREENWIDTH - loading.width) / 2,
    (SCREENHEIGHT - loading.height) / 2
  );
}
...
function updateTutorial() {
  ...
  if (page < 0) fadeOut(menu);
  if (page >= tutinsts.length) fadeOut(initGame);
}
