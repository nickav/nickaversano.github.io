...
function clearInstances() {
  for (var i = 0, il = instances.length; i < il; i++) instances[i].destroy();
}

function clearInsts() {
  for (var i = 0, il = insts.length; i < il; i++) insts[i].destroy();
}

function destroyInstances() {
  if (destroys.length > 0) {
    for (var d = 0, dl = destroys.length; d < dl; d++) destroys[d].remove();
    destroys.length = 0;
  }
}
