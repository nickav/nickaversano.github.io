  ...
  this.shoot = function() {
    if (this.absorb != 0) {
      ...
      var a = new Beam(
        this.x,
        -10,
        [shiplaser0, shiplaser1, shiplaser2],
        1,
        54,
        this.y + 40
      );
...
Ship.prototype.move = function() {
  ...
  if (this.flash) {
    ...
    this.alpha =
      (Math.sin(
        (this.timer * ((this.timer / (this.flashtime * 2)) * 50)) /
          this.flashtime
      ) *
        0.33 +
        0.33) *
  ...
  if (!this.flash) {
    for (var i = 0, il = groups.length; i < il; i++) {
      if (
        (groups[i].stick || groups[i].blocks.length == 4) &&
        !groups[i].number == 0
      ) {
        var group = groups[i];
        if (
  ...
  if (!this.flash && groups.length > 0 && this.closeToBlock) {
    ...
    for (var i = 0, il = this.closeBlocks.length; i < il; i++) {
      for (var j = 0, jl = this.closeBlocks[i].blocks.length; j < jl; j++) {
        var group = this.closeBlocks[i],
          block = group.blocks[j];

        if (
          this.x + woset + 1 < block.x &&
          collision(this, block, this.rSpeed + this.accel, 0, woset, hoset)
        ) {
        ...
        if (
          this.x - woset - 1 > block.x &&
          collision(this, block, -this.lSpeed - this.accel, 0, woset, hoset)
        ) {
          while (!collision(this, block, -1, 0, woset, hoset)) this.x--;
          this.lSpeed = 0;
        }
        if (
          this.y + hoset + 1 < block.y &&
          collision(this, block, 0, this.dSpeed + this.accel, woset, hoset)
        ) {
        ...
        if (
          this.y - hoset - 1 > block.y &&
          collision(this, block, 0, -this.uSpeed - this.accel, woset, hoset)
        ) {
          while (!collision(this, block, 0, -1, woset, hoset)) this.y--;
          this.uSpeed = 0;
        }

        if (collision(this, block, 1, 0, woset, hoset)) canMoveRight = false;
        if (collision(this, block, -1, 0, woset, hoset)) canMoveLeft = false;
        if (collision(this, block, 0, 1, woset, hoset)) canMoveDown = false;
        if (collision(this, block, 0, -1, woset, hoset)) canMoveUp = false;

        if (collision(block, this, 0, 0, GRIDSIZE * 0.5, GRIDSIZE * 0.5))
          stuck = true;
          ...
          if (collision(this, block, 1, 0, woset, hoset))
            if (pressingRight) {
              if (this.moveRightDelay < this.moveDelay) this.moveRightDelay++;
                ...
                if (group.stick) {
                  if (
                    group.canMove(GRIDSIZE, 0) &&
                    group.canMove(0, GRIDSIZE)
                  ) {
                    group.x += GRIDSIZE;
                    group.moveX();
                  }
  ...
  if (this.shootFast) {
    if (this.shootFastTimer < this.shootFastTime) this.shootFastTimer++;
    else {
      this.shootFastTimer = 0;
      this.shootFast = false;
    }
  ...
  if (!this.flash)
    for (var i = 0, il = enemybullets.length; i < il; i++)
      if (collision(enemybullets[i], this, 0, 0, 6, 4)) {
        if (
          (this.absorb == -1 ||
            typeof enemybullets[i].indestructable == 'undefined') &&
          this.invtimer == 0
        )
          this.damage();
        if (typeof enemybullets[i].indestructable == 'undefined')
          enemybullets[i].destroy();
        else if (this.absorb > 0) {
          this.absorb = 0;
          this.invtimer = 4;
          this.shoot();
        }
  ...
  if (this.shot) {
    if (this.guns.yoffset > -1) this.guns.yoffset -= 2 + this.shootFast * 4;
    else this.shot = false;
  } else {
    if (this.guns.yoffset < 0) this.guns.yoffset += 1 + this.shootFast * 2;
    else if (this.guns.yoffset != 0) this.guns.yoffset = 0;
  }
...
function Bullet(x, y) {
  ...
  this.move = function() {
    ...
    else if (!this.alive) this.alive = true;
    if (this.y > SCREENTOP - 12 * 2) this.y -= this.speed;
    else this.destroy();
  };
  this.destroy = function() {
    destroyInst(this);
...
function Block(x, y, direction, group, length) {
  ...
  switch (this.group.number) {
    case 0:
      this.image(cyanblock);
      break;
    case 1:
      this.image(orangeblock);
      break;
    case 2:
      this.image(greenblock);
      break;
    case 3:
      this.image(yellowblock);
      break;
    case 4:
      this.image(redblock);
      break;
    case 5:
      this.image(purpleblock);
      break;
    case 6:
      this.image(blueblock);
      break;
  }
  ...
  this.destroy = function() {
    destroyInst(this);
    if (this.group)
      this.group.blocks.splice(this.group.blocks.indexOf(this), 1);
  ...
  if (this.square)
    for (var i = 0, il = squares.length; i < il; i++)
      for (var j = 0, jl = squares[i].length; j < jl; j++) {
        ...
        if (
          (this.x == square.x &&
            (this.y + GRIDSIZE == square.y || this.y - GRIDSIZE == square.y)) ||
          (this.y == square.y &&
            (this.x + GRIDSIZE == square.x || this.x - GRIDSIZE == square.x))
        )
          square.square = true;
      }
...
Block.prototype.canMove = function(newX, newY) {
  if (
    this.x + newX < SCREENLEFT ||
    this.x + newX >= SCREENWIDTH ||
    this.y + newY >= SCREENHEIGHT
  )
    return false;
  for (var i = 0, il = blocks.length; i < il; i++) {
    if (blocks[i] != this) {
      ...
      if (
        (this.x + newX == other.x &&
          this.y + newY == other.y &&
          (this.group != other.group || !other.canMove(newX, newY))) ||
        this.y + newY >= SCREENHEIGHT ||
        this.x + newX >= SCREENWIDTH ||
        this.x + newX < SCREENLEFT
      )
  ...
  for (var i = 0, il = groups.length; i < il; i++)
    for (var j = 0, jl = groups[i].blocks.length; j < jl; j++) {
      if (groups[i].blocks[j] != this) {
        if (
          this.x + newX == groups[i].blocks[j].x &&
          this.y + newY == groups[i].blocks[j].y
        ) {
          // || this.y+newY>=SCREENHEIGHT || this.x+newX>=SCREENWIDTH || this.x+newX<SCREENLEFT
          if (this.group == groups[i]) {
            var tblocks = this.getTouching();
            for (var k = 0, kl = tblocks.length; k < kl; k++)
              if (
                tblocks[k].x + newX >= SCREENWIDTH ||
                tblocks[k].x + newX < SCREENLEFT ||
                tblocks[k].y + newY >= SCREENHEIGHT ||
                !tblocks[k].canMove(newX, newY)
              )
                return false;
          } else return false;
        }
...
Block.prototype.getTouching = function() {
  ...
  for (var i = 0, il = this.group.blocks.length; i < il; i++) {
    if (this.group.blocks != this) {
      ...
      if (
        (this.x == other.x &&
          (this.y + GRIDSIZE == other.y || this.y - GRIDSIZE == other.y)) ||
        (this.y == other.y &&
          (this.x + GRIDSIZE == other.x || this.x - GRIDSIZE == other.x))
      )
        touches.push(other);
    }
  }
...
Block.prototype.moveTouches = function(newX, newY) {
  if (!this.touching()) {
    ...
    for (var i = 0, il = touches.length; i < il; i++)
      if (
        !touches[i].canMoveBlock(newX, newY) ||
        !touches[i].canMoveBlock(0, GRIDSIZE)
      ) {
        can = false;
        break;
      }
...
Block.prototype.getAllTouches = function() {
  ...
  for (var i = 0, il = touches.length; i < il; i++) {
    ...
    for (var j = 0, jl = t.length; j < jl; j++)
      if (!findIn(touches, t[j])) touches.push(t[j]);
  }
  return touches;
};
// \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
Group.prototype.move = function() {
  if (!this.checkedblocks) {
    var passed = true;
    for (var i = 0, il = this.blocks.length; i < il; i++) {
      if (this.blocks[i].x >= SCREENWIDTH) {
        this.x -= GRIDSIZE;
        this.moveX();
        passed = false;
        break;
      }
      if (this.blocks[i].x < SCREENLEFT) {
        this.x += GRIDSIZE;
        this.moveX();
        passed = false;
        break;
  ...
  if (time == 0) {
    if (this.stick) {
      if (this.y < SCREENHEIGHT - this.height && this.canMove(0, GRIDSIZE)) {
        if (this.oldx != this.x) this.moveX();
      ...
      for (var i = 0, il = this.blocks.length; i < il; i++) {
        if (
          this.blocks[i].y < SCREENHEIGHT - this.blocks[i].height &&
          this.blocks[i].canMoveBlock(0, GRIDSIZE)
        ) {
          if (!this.blocks[i].touching()) this.blocks[i].y += GRIDSIZE;
          else {
            var touches = this.blocks[i].getAllTouches(),
              can = true;
  ...
  if (this.blocks.length == 0) this.destroy();
};
...
Group.prototype.canMove = function(newX, newY) {
  ...
  for (var i = 0, il = groups.length; i < il; i++) {
    if (groups[i] != this) {
      //&& collision(this, groups[i], 0, 1)
      for (var j = 0, jl = groups[i].blocks.length; j < jl; j++) {
        var other = groups[i].blocks[j];
        for (var k = 0, kl = this.blocks.length; k < kl; k++) {
          if (
            (this.blocks[k].x + newX == other.x &&
              this.blocks[k].y + newY == other.y) ||
            this.blocks[k].x + newX >= SCREENWIDTH ||
            this.blocks[k].x + newX < SCREENLEFT ||
            this.blocks[k].y + newY >= SCREENHEIGHT
          )
            return false;
        }
      }
...
function EnemyBullet(x, y, image, speed) {
  ...
  this.move = function() {
    if (this.y < SCREENHEIGHT + this.height + this.speed) this.y += this.speed;
    else this.destroy();
  };
  this.destroy = function() {
    destroyInst(this);
...
function Enemy(x, y) {
  ...
  if (this instanceof Enemy) {
    this.fire = new Fire(
      this,
      [enemy2fire0, enemy2fire1, enemy2fire2, enemy2fire3, enemy2fire1],
      0.75,
      21,
      18,
      9,
...
Enemy.prototype.move = function() {
  ...
  if (
    this.movement != 0 &&
    this.y > this.maxheight - this.dist &&
    this.yforce > 0
  ) {
    this.yforce = 0;
  }
  ...
  if (this.y > ship.y + ship.height * 0.5 && this.movement == 0)
    this.yforce = 12;
  ...
  if (this.movement < 0) {
    ...
    if (
      this.movementtimer >= this.movementtime - FRAMERATE * 0.5 &&
      this.movementtime % 3 == 0
    ) {
      this.xforce = 0;
      this.yforce = 4 * ((Math.floor(this.movementtimer / 2) % 2) * 2 - 1);
    }
  } else {
    this.movetime++;
    if (this.movetime >= this.stoptime && this.movement != 0) {
      this.movetime = 0;
      this.movement = -1;
...
Enemy.prototype.update = function() {
  if (bullets.length > 0) {
    for (var b = 0, bl = bullets.length; b < bl; b++)
      if (collision(this, bullets[b], 0, 0, 4, 8 - this.yspeed / 2)) {
        ...
        if (typeof bullets[b].indestructable == 'undefined')
          bullets[b].destroy();
        else this.health = 0;
        if (this.health == 0) {
          addScore(level * this.value);
          localStorage.ekill = ++enemiesKilled; //!!
          if (enemiesKilled > ENEMIESTOKILL && skipToLevel && !waveMode) {
            waveMode = true;
            tellthem = true;
          }
          this.explode();
  ...
  if (this.shootTimer < this.shootTime) this.shootTimer++;
    ...
    this.shootTimer = 0;
    this.shootTime = Math.floor(Math.random() * this.chance);
  }
  ...
  if (this.xforce != 0)
    this.xspeed = incTo(this.xspeed, this.accel, this.xforce);
  else this.xspeed = incTo(this.xspeed, this.frict, 0);
  if (this.yforce != 0)
    this.yspeed = incTo(this.yspeed, this.accel, this.yforce);
  else this.yspeed = incTo(this.yspeed, this.frict, 0);

  this.dist = stoppingDist(this.xspeed, this.frict) + Math.abs(this.xspeed);
  if (
    (!this.turn &&
      this.x >= SCREENWIDTH - this.width - this.dist &&
      this.xforce > 0) ||
    (this.x <= SCREENLEFT + this.dist && this.xforce < 0 && this.size != 1.5)
  ) {
    this.prevxforce = this.xforce;
    this.xforce = 0;
    this.turn = true;
  }
  ...
  if (this.movement == -1 && this.y < this.maxheight - this.dist)
    this.yforce = this.speed;
...
function Enemy2(x, y) {
  ...
  this.shoot = function() {
    new EnemyBullet(
      this.x + 5 - 11 * !this.guns,
      this.y + 12,
      enemy2bullet,
      12
    );
    new EnemyBullet(
      this.x + this.width - 32 + 11 * !this.guns,
      this.y + 12,
      enemy2bullet,
      12
    );
...
Enemy2.prototype.move = function() {
  ...
  if (this.movement == 0) {
    if (Math.abs(this.x - ship.x) < GAMEWIDTH / 3) {
      ...
      if (this.y < ship.y + ship.height * 0.5) {
        var dir = directionTo(this, ship) * DEGTORAD;
        this.xforce = this.speed * Math.cos(dir);
        this.yforce = Math.min(
          Math.max(this.speed * Math.sin(dir) * 1.25, this.speed * 0.4),
          this.speed
        );
  ...
  if (
    (!this.guns && this.timer >= (FRAMERATE * 4) / 9) ||
  ...
  if (this.movementtimer < this.movementtime) this.movementtimer++;
    ...
    if (this.chargetimer < this.chargetime) {
      ...
      if (
        ...
        yy = ship.y - this.y - ship.height;
      var a = new Beam(
        this.x + 13,
        this.y + this.height - 5,
        [enemy3laser0, enemy3laser1, enemy3laser2],
        1,
        49,
        yy + ship.height
      );
  ...
  if (this.movement < 0 && this.resumetimer == 0) {
    if (this.decide)
      for (var i = 0, n = bullets.length; i < n; i++) {
        if (
          collisionRect(bullets[i], this.x, 0, this.width / 2, SCREENHEIGHT)
        ) {
          ...
          if (this.x - SCREENLEFT >= this.width) this.xforce = -this.speed;
          else this.xforce = this.speed;
          this.decide = false;
          this.dectime = 5;
        }
    ...
    else if (!this.decide && this.dtimer == 0) {
      this.xforce = 0;
      this.dtimer = this.dtime;
    }
...
Animation.prototype.move = function() {
  if (this.timer + this.speed < 1) this.timer += this.speed;
...
function Explosion(x, y, size, speed) {
  this.superClass(
    x,
    y,
    [explosion0, explosion1, explosion2, explosion3, explosion4, explosion5],
    speed || 1,
    64 * (size || 1),
    64 * (size || 1)
  );
...
function PowerUp(x, y, number) {
  this.superClass(
    x,
    y,
    [powerup0, powerup1, powerup2, powerup3, powerup5, powerup6, powerup7],
    0.25,
    24,
    24
  );
      ...
      switch (this.number) {
        case 0:
          ...
          if (shipLives > MAXSHIPLIVES) shipLives = MAXSHIPLIVES;
            ...
            new Grower(ship.x + ship.width * 0.5, ship.y + ship.height * 0.5);
          break;
        case 1:
          ship.shootFast = true;
          break;
        case 2:
          ...
          if (can) {
            ...
            s.grow = s.fade = 0;
            s.move = function() {
              var extra = Math.sin(this.a++ / 3) * 15 - 10;
              if (this.oldShield != ship.shield) {
                this.oldShield = ship.shield;
                this.grow = 40;
              }
              if (this.grow > 0) this.grow -= 4;
              if (this.fade < 1) this.fade += 0.1;
              this.width = this.aW + extra + this.grow;
              this.height = this.aH + extra + this.grow;
              this.alpha = (ship.alpha * this.fade * ship.shield) / 6;
              this.x = ship.x - this.width * 0.5 + ship.width * 0.5;
              this.y = ship.y - this.height * 0.5 + ship.height * 0.5 + 2;
              if (this.alpha <= 0 && ship.shield == 0) {
                this.destroy();
                PowerUp.items[2]--;
              }
            };
...
function Shine(x, y) {
  ...
  this.move = function() {
    if (this.fadein) {
      if (this.alpha < 0.8) this.alpha += 0.25;
      else {
        this.fadein = false;
        this.alpha = 0.8;
      }
...
function Button(x, y, images, press) {
  ...
  this.pressing = function() {
    return (
      ...
      collisionRectangles(mX, mY, 1, 1, this.x, this.y, this.width, this.height)
    );
  }; //**
  this.press = press;
  this.pressed = false;
  ...
  this.move = function() {
    if (this.pressing() && !this.pressed) {
      this.image(this.images[1 + this.index]);
      this.press();
      play(snd_click);
      this.pressed = true;
    } else if (!this.pressing() && this.pressed) {
      this.image(this.images[0 + this.index]);
      this.pressed = false;
    }
  };
  this.cycle = function() {
...
Button.removeAll = function() {
  for (var i = 0, n = buttons.length; i < n; i++) buttons[i].remove();
};
...
Absorber.prototype.move = function() {
  if (!this.spin) {
    ...
    if (this.index == 6) this.spin = true;
  } else this.rotation += 36;

  if (this.parent.absorb < 0) this.die = true;
...
Expander.prototype.move = function() {
  ...
  if (this.width > this.img.width * 2 && this.parent.cantShoot)
    this.parent.cantShoot = false;
...
function Beam(x, y, images, animationSpeed, width, height) {
  ...
  this.destroy = function() {
    if (enemybullets.indexOf(this) > -1)
      enemybullets.splice(enemybullets.indexOf(this), 1);
...
HighscoreTable.prototype.index = function(score) {
  for (var i = 0; i < 10; i++)
    if (score > localStorage.getItem(this.name + 'score' + i)) break;
  if (i == 10) return -1;
  return i;
};
// \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
