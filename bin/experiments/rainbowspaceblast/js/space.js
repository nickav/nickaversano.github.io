...
var SENS_X = 20,
  SENS_Y = 25; //accelerometer sensetivity
...
function init() {
  ...
  for (var i = 0; i < Particle.amount; i++)
    new Particle(
      MIN_X + X_RANGE * Math.random(),
      MIN_Y + Y_RANGE * Math.random()
    );
  ...
  this.move = function() {
    ...
    if (x < -speed || x > WIDTH + speed || y < -speed || y > HEIGHT + speed) {
      ...
      x =
        MIN_X +
        X_RANGE * Math.random() -
        (speed / MAX_SPEED) * Math.cos(d) * MIN_X * 2;
      y =
        MIN_Y +
        Y_RANGE * Math.random() -
        (speed / MAX_SPEED) * Math.sin(d) * MIN_Y * 2;
      xprev = x;
      yprev = y;
      life = 1 + speed / 50;
      z = (1 - MIN_Z) * Math.random() + MIN_Z;
